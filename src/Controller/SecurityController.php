...
{
    /**
     * @Route(name="api_login", path= "/authentication_token")
     * return JsonResponse
     */
    ...
    public function forgottenPass(Request $request, UserRepository $usersRepo, \Swift_Mailer $mailer, TokenGeneratorInterface $tokenGenerator){
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            ->setTo($user->getEmail())
            ->setBody(
                "<p>Bonjour,</p><p>Une demande de réinitialisation de mot de passe a été effectuée pour le site Atypikhouse.fr. Veuillez cliquer sur le lien suivant : " . $url .'</p>',
                'text/html'
                )
            ;
    ...
    public function resetPassword($token, Request $request, UserPasswordEncoderInterface $passwordEncoder){
        ...
        $user = $this->getDoctrine()->getRepository(User::class)->findOneBy(['rest_token' => $token]);
